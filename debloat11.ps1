...
"@ 
Write-Host "Windows 11/10 Debloater | made by alexplaygithub" -ForegroundColor Green
    ...
    $InputReboot = Read-Host "Do you wish to reboot now? [y/n]"
       switch ($RebootNow)
            ...
            $WifiSense3 = "HKLM:\SOFTWARE\Microsoft\WcmSvc\wifinetworkmanager\config"
            If (!(Test-Path $WifiSense1)) {
                New-Item -Path $WifiSense1
            }
            Set-ItemProperty -Path $WifiSense1 -Name Value -Value 0 
            If (!(Test-Path $WifiSense2)) {
                New-Item -Path $WifiSense2
            }
            Set-ItemProperty -Path $WifiSense2 -Name Value -Value 0 
            Set-ItemProperty -Path $WifiSense3 -Name AutoConnectAllowedOEM -Value 0 
            ...
            $DataCollection3 = "HKLM:\SOFTWARE\Wow6432Node\Microsoft\Windows\CurrentVersion\Policies\DataCollection"    
            If (Test-Path $DataCollection1) {
                Set-ItemProperty -Path $DataCollection1 -Name AllowTelemetry -Value 0 
            }
            If (Test-Path $DataCollection2) {
                Set-ItemProperty -Path $DataCollection2 -Name AllowTelemetry -Value 0 
            }
            If (Test-Path $DataCollection3) {
                Set-ItemProperty -Path $DataCollection3 -Name AllowTelemetry -Value 0 
            }
            ...
            If (Test-Path $CloudStore) {
                ...
                Start-Process -Name "Explorer.exe" -Wait
            }
            Clear-Host
            ...
            $Edge = "HKCR:\AppXd4nrz8ff68srnhf9t5a8sbjyar1cr723_"
            If (Test-Path $Edge) {
                Set-Item -Path $Edge -Name AppXd4nrz8ff68srnhf9t5a8sbjyar1cr723_ 
            }
            Clear-Host
